...
describe('HelloWorldService', () => {
  ...
  it('should say hello with the table name if found (cache miss)', async () => {
    ...
    const call = { request } as unknown as ServerUnaryCall<HelloRequest, HelloReply>;
    ...
    expect(reply.getMessage()).toBe(`Hello ${mockName} from gRPC using table: ${mockTableName}`);
  });
  ...
  it('should say hello with table: undefined if no record is found (cache miss)', async () => {
    ...
    const [error, reply] = callback.mock.calls[0];
    expect(error).toBeNull();
    expect(reply.getMessage()).toBe(`Hello ${mockName} from gRPC using table: undefined`);
  });
  ...
  it('should return cached HelloReply if cache hit occurs', async () => {
    ...
    cachedReply.setMessage(`Hello ${mockName} from gRPC using table: fromCache`);
    ...
    const [error, reply] = callback.mock.calls[0];
    expect(error).toBeNull();
    expect(reply.getMessage()).toBe(`Hello ${mockName} from gRPC using table: fromCache`);
  });
